...
posts = function(listen) {
    listen.post('/fullText', function (request, response) {
       screwcore.push(request.body.title || '', request.body.fullText || '', request.body.source || '', function(error) {
          error && winston.info('Error on creating a fullText. ' + error);
       });
    
       response.json('fullText send to mongo');
    });
};
